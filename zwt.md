...
~/codspeed/sessions/cod-2931/
  .envrc                 generated: defers to the workspace's, then exports the root
...
### The session root comes from direnv, not from the multiplexer

The earlier plan had `zwt` export `WORKSPACE_ROOT` into the zellij server so panes
inherited it. That couples the root to milestone 2 for no reason. Instead **the session's
own `.envrc` exports it**, so any shell that `cd`s into the session gets it — a hand-made
zellij session, a bare terminal, `direnv exec`, all the same:

```sh
# ~/codspeed/sessions/cod-2931/.envrc   (generated by zwt, git-ignored anyway)
# generated by zwt — `zwt sync --fix` rewrites it
source_env '/home/guillaume/codspeed/.envrc'   # keeps `use flake ./flake`
export WORKSPACE_ROOT="$(expand_path .)"
```

`expand_path .` resolves against the `.envrc`'s own directory, so the value is the
session path however direnv got there. The export comes last so the workspace's own
`.envrc` cannot win.

Member repos need the value too, because `cicr` & friends resolve
`${WORKSPACE_ROOT:-~/codspeed}/<repo>`. Repo `.envrc`s are inconsistent about this today
(`platform` has `source_env ../.envrc`, `codspeed-rust` does not) and they are globally
git-ignored, so hydration normalises the copy it drops into the session: prepend
`source_up_if_exists` unless the file already reaches its parent. Idempotent, and only
ever touching the session's copy — done before `direnv allow`, whose approval is keyed on
the contents.

Fallback for the cases direnv cannot cover — a symlinked (non-member) repo, whose
physical `.envrc` is the main checkout's and therefore resolves `WORKSPACE_ROOT` to
`~/codspeed` — `cdr` resolves its root in this order:
...
normalise the copied .envrc (source_up_if_exists)
direnv allow <path>
```

That is the whole of it. Installed dependencies and generated code are **not** copied:
they are the repo's own `post-checkout` hook's business, per repo, and a hook that
installs its own `node_modules` is both more correct and less surprising than reflinking
someone else's.
...
longer what zwt generates, a missing marker, a member `.envrc` that never reaches the
session root, worktrees deleted by hand, branches no longer matching the key.
...
- [ ] `cd` into the session: `WORKSPACE_ROOT` is the session, `cdr docs` lands in the
      symlink, `cdr platform` in the worktree, and `cdg` jumps to the enclosing repo root.
...
- `zwt <id>` attaches, creating the zellij session and its tabs if absent. Attach never
  mutates the session directory.
- `ZELLIJ_SESSION_NAME` == id, so panes self-identify — the join key milestone 3 needs.
- `WORKSPACE_ROOT` still comes from direnv (milestone 1), not from the zellij server; a
  pane that direnv cannot cover falls back to the `.zwt/session.json` marker walk.
...
niri, no notify-send, and no zsh hook reading `ZELLIJ_SESSION_NAME`.

## Verified primitives

Everything here was checked on this machine (zellij 0.44.3, niri, kitty). Milestone 1
uses none of it; it is recorded for 2 and 3.
...
- **`.envrc` normalisation** vs. repos that grow a `source_env ../.envrc` upstream — the
  prepend must stay idempotent and detect both forms. (1)

...
//
// Submenus are pushed onto a StackView rather than flown out sideways, which is
// the shape caelestia-dots/shell and DankMaterialShell use. A flyout has to be
// reached by crossing the rows between it and the pointer, any one of which
// takes the hover and closes it, and the stack also keeps every level's
// `QsMenuOpener` alive — see the gotcha about repointing one.
import QtQuick

...
Rectangle {
    ...
    // Pill texts are "<glyph> <value>", and the glyph is split out of the
    // string so it can be drawn by Glyph: private-use codepoints have no
    // relation to the line box the label is laid out in, so leaving them in
    // the Text is what makes single-icon pills look nudged. Splitting here
    // covers script-fed texts (Claude, recorder) too, with nothing per pill.

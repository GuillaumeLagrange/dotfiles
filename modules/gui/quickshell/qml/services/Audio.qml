pragma Singleton
// Pipewire's own graph, which is what pavucontrol was open for: the output and
// input devices, the default of each, and the per-application streams.
import QtQuick
import Quickshell
...
Singleton {
    ...
    readonly property var streams: root.nodes.filter(node => node.audio && node.isStream && node.isSink)
    readonly property var recorders: root.nodes.filter(node => node.audio && node.isStream && !node.isSink)

pragma Singleton
// BlueZ through quickshell's own client, which is everything blueman-applet was
// in the tray for: the adapter switch, the paired devices, and their batteries.
import QtQuick
import Quickshell
...
Singleton {
    ...
    readonly property var knownDevices: root.devices.filter(dev => dev.paired || dev.bonded)
    readonly property var newDevices: root.devices.filter(dev => !dev.paired && !dev.bonded)

pragma Singleton
// CPU, memory/swap and disk sampling, ported from metrics.py. FileView reads
// procfs directly (no `cat` fork); disk free space has no statvfs binding in
// QML, so that one sample uses Config.df through a Process instead.
import QtQuick
import Quickshell
...
Singleton {
    ...
    property var cpu: ({ usage: "00", tooltip: "" })
    property var memswap: ({ text: "", class: "ok", tooltip: "" })
    property var disk: ({ free: "", tooltip: "" })
    ...
    function _sampleCpu(): void {
        ...
        if (root._prevStat === null) {
            ...
            // next reload, same as Cpu()'s constructor snapshot in metrics.py.
            root._prevStat = cur;
            return;
        }
        ...
        const usage = pct("cpu");
        root._prevStat = cur;
        root.cpu = { usage: String(usage).padStart(2, "0"), tooltip };
    }
    ...
    function _sampleMemSwap(): void {
        ...
        root.memswap = {
            ...
            // Disk spill past zram is the signal worth surfacing, but nothing
            // currently bumps this off "ok" (matches metrics.py as written).
            class: "ok",

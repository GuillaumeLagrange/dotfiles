...
#[derive(Parser)]
#[command(name = "wt", version, args_conflicts_with_subcommands = true)]
struct Cli {
    #[command(subcommand)]
    command: Option<Command>,

    /// Session to attach to. Without one, pick from the registry.
...
enum Command {
    ...
    New {
        ...
        repos: Vec<String>,
        /// Start with no members: just the mirror, to `wt add` into.
...
fn run() -> Result<()> {
    ...
    let cfg = Config::load()?;
    match cli.command {
        None => cmd::attach::run(&cfg, cli.id.as_deref()),
        Some(Command::New {
            name,
            title,
            at,
        ...
        Some(Command::Path { id, exact }) => cmd::path::run(&cfg, id.as_deref(), exact),
    }
}

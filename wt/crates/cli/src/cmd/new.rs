...
use wt_core::{envrc, git, mirror, session, util, workspace};
...
pub fn run(cfg: &Config, args: Args) -> Result<()> {
    ...
    anyhow::ensure!(
        reg.get(&id).is_none(),
        "session `{id}` already exists — `wt {id}` prints its root"
    );
    ...
    if let Err(err) = hydrate::allow_direnv(&path) {
        eprintln!("wt: could not allow direnv in the session root: {err:#}");
    }
    ...
    for (repo, err) in &failed {
        eprintln!("wt: {repo}: {err:#}");
    }
    if !failed.is_empty() {
        eprintln!(
            "wt: {} repo(s) left as symlinks; `wt add <repo>` retries one",
            failed.len()
        );
    }
    Ok(())
...
pub fn check_out(cfg: &Config, session: &Path, repo: &str) -> Result<()> {
    ...
    if let Some(warning) = git::add_worktree(&repo_path, &worktree, &base)? {
        eprintln!("wt: {repo}: the checkout succeeded but a git hook failed:\n{warning}");
    }

    let report = hydrate::hydrate(cfg, &git::main_worktree(&repo_path)?, &worktree)?;
    for (path, err) in &report.failed {
        eprintln!("wt: {repo}: could not copy {path}: {err}");
    }
    Ok(())
}

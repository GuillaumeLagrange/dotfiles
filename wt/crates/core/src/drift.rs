...
use crate::{envrc, git, hydrate, mirror, session, util, zellij};
...
pub enum Drift {
    ...
    MarkerMissing,
    /// A member with no tab in the running zellij session — it joined while the
    /// session was detached, or the tab was closed.
    TabMissing {
        repo: String,
    },
    ...
    pub fn describe(&self) -> String {
        match self {
            ...
            Self::SessionEnvrcStale => {
                ".envrc is not the one wt generates for a session".into()
            }
            Self::DirenvNotAllowed => "direnv has not been allowed here".into(),
            Self::MarkerMissing => format!("{} is missing", session::MARKER),
            Self::TabMissing { repo } => format!("`{repo}` has no tab in the zellij session"),
...
pub fn fix(cfg: &Config, session: &Session, item: &Drift) -> Result<()> {
    match item {
        ...
        Drift::TabMissing { repo } => {
            zellij::new_tab(&session.id, &session.path.join(repo), repo)?;
        }

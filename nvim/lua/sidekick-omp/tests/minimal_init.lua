-- Minimal init for running the sidekick-omp test suite headless, independent of
-- the full user config. Run the suite with `just test` from the plugin root.

local here = vim.fn.fnamemodify(debug.getinfo(1, 'S').source:sub(2), ':p:h')
local pkg_dir = vim.fn.fnamemodify(here, ':h') -- lua/sidekick-omp/
...
-- expose *only* the `sidekick-omp.*` package via a scoped searcher.
...
-- Resolve `sidekick-omp` and `sidekick-omp.<mod>` to files under pkg_dir, and
-- nothing else. Leaves every other module to the default searchers.
table.insert(package.loaders, 1, function(name)
  local sub = name:match('^sidekick%-omp%.(.+)$')
  if name ~= 'sidekick-omp' and not sub then
    return nil
  end
  ...
  return ('\n\tno sidekick-omp file for %q under %s'):format(name, pkg_dir)
end)

vim.opt.swapfile = false

-- Keep the suite out of the real session registry under ~/.omp.
vim.env.OMP_NVIM_BRIDGE_DIR = vim.fn.tempname() .. '/nvim-bridge'

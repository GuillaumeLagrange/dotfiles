...
`wt` is the tool. `zsm` stays as it is (dumb attach-or-create by name, fzf picker) for the
zellij sessions that are not wt sessions.
...
| ------------------------ | ---------------------------------------------------------------------------------- | ------- |
| **1 — session dirs**     | mirrored session root, member selection, branch resolution, hydration, cleanup, `cdr` | done    |
| **2 — zellij**           | attach, one tab per member, tab naming, cross-session tab add                       | current |
| **3 — agent overwatch**  | Claude hooks, state files, dashboard plugin, `jump`, notifications                  | later   |

Everything written for 3 (`crates/plugin/`, `core/{agents,jump,notify}.rs`,
`cli/cmd/{agents,hook,jump}.rs`, the `wt-dashboard` wrapper and its `Ctrl+b a` binding, the
mako rule) is **out of the tree**; it comes back when that milestone starts. The design
notes for it are kept below, under their own headings, so nothing has to be re-derived.
...
~/codspeed/sessions/cod-2931/
  ...
  .wt/session.json      the marker that makes the directory self-describing
...
Discovery is a **registry**, `~/.local/state/wt/sessions.json`, kept deliberately thin:
...
Nothing about CodSpeed is compiled into `wt`: no repo names, no ticket prefix, no
`~/codspeed` default. Workspace, sessions root and the selectable repos all come from
`~/.config/wt/config.toml`, which is imperative — written by hand on each machine, kept
...
```sh
# zsm, when `wt path --exact <name>` resolves
...
file wt generates. The environment is the only additive channel, and it needs no file at
all.
...
`$ZELLIJ_SESSION_NAME` is the registry key, so `wt path --exact` on it gives the root
...
1. nearest ancestor of the **logical** `$PWD` containing `.wt/session.json`
...
So the marker file `<session>/.wt/session.json` is part of the session layout: it is
...
`wt` itself resolves "the session I am in" the same way, preferring `$PWD` over the
resolved cwd whenever the two still describe the same directory. That is what makes
`wt path`, `wt add` and `wt sync` work from inside a symlinked member.

## Member selection

`wt new <session-name>` shows an fzf multi-select over the **pre-approved repo list**,
not over everything in `~/codspeed` — forks, vendored trees and scratch checkouts never
show up.

That list is `repos` in `~/.config/wt/config.toml`, which is written **by hand** and
...
Non-selected repos become symlinks; `wt add <repo>` swaps one for a worktree later, on
the same terms as if it had been picked. Picking nothing is valid: the session is then a
complete mirror, ready to `wt add` into.
...
answer than anything wt could guess, and `wt promote` is there for when a worktree does
...
The list is `hydrate` in `~/.config/wt/config.toml`, on the same grounds as `repos`:
...
`wt rm <id>` is the guarded teardown, and it is the half of the milestone that must not
...
`wt sync` reports drift, `wt sync --fix` repairs it: stale mirror symlinks (repos
cloned or removed since creation), un-allowed direnv, an `.envrc` that is missing or no
longer what wt generates, a missing marker, worktrees deleted by hand, branches no longer
matching the key.

## CLI surface

```
wt                       fzf picker over sessions -> attach
wt <id>                  attach, creating the zellij session and its tabs if absent
wt new <branch|id>       scan, pick members, worktrees + mirror + hydrate
wt add <repo> [branch]   symlink -> worktree, hydrate
wt promote <repo>        worktree -> symlink, main checkout switches to the branch
wt rm [<id>]             guarded teardown (dirty tree / unpushed / stashes; --force)
wt ls                    sessions, members, branches
wt path [<id>]           the session root, for shell use (`cd "$(wt path cod-2931)"`)
...
`wt path` is what scripts and shell functions use; `wt` itself is for hands.

## Validation checklist

- [ ] `wt new cod-2931-…` with two members: worktrees real, every other repo a symlink,
      `.envrc` + `.wt/session.json` present, direnv allowed.
- [ ] `wt cod-2931`: one tab per member, named after it and opened in it; `WORKSPACE_ROOT`
      is the session in every pane, `cdr docs` lands in the symlink, `cdr platform` in the
      worktree, `cdg` jumps to the enclosing repo root.
- [ ] `wt cod-2931` again, and from inside another session: no duplicate tabs, and the
      second one switches rather than refusing.
...
- [ ] `wt add codspeed` swaps a symlink for a worktree in place, path unchanged.
- [ ] `wt promote platform` swaps back and leaves the main checkout on the branch.
- [ ] `wt rm` refuses on a dirty tree, unpushed commits and a stash; `--force` proceeds.
- [ ] after `wt rm`, `~/codspeed/<repo>` is intact — no symlink was followed.
- [ ] `wt sync` sees a hand-deleted worktree and a repo cloned after creation, `--fix`
      repairs both.
...
One tab per member repo, named after the repo, opened in it. **No layout opinion** —
splits are mine to make, and nothing ever rewrites a tab that exists.
...
Claude hook (SessionStart|UserPromptSubmit|PostToolUse|Notification|Stop)
  -> $XDG_RUNTIME_DIR/wt/agents/<claude_session_id>.json
     { state, zellij_session, pane_id, cwd, nvim_sock, pid, repo, branch, ts }
  -> zellij pipe --name wt-refresh          (nudge, best effort)

plugin: set_timeout(2s) + pipe -> run_command("wt agents --json")
jump:   run_command("wt jump <id>")         ONE implementation, shared with mako
```
...
hook Notification:
  ...
  notify-send --app-name=wt \
    -h string:x-canonical-private-synchronous:wt-<agent> \
    -A default=Jump "cod-2931 / codspeed-node" "waiting for input"
  (backgrounded; action 'default' -> wt jump <agent>)

mako: "app-name=wt" { on-button-left=invoke-default-action }
...
```
wt                       fzf picker over sessions -> attach          (2)
wt <id>                  attach, create zellij session + tabs        (2)
wt jump <agent>          the one jump implementation                 (3)
wt agents [--json]       state for the plugin, mako, anything else   (3)
wt hook <event>          what Claude's hooks call                    (3)
```
...
Rust workspace at `~/dotfiles/wt/` (root level, like `nvim/`), built with cargo for now
...
Milestone 1 keeps `wt`'s runtime PATH down to `coreutils direnv fzf git` — no zellij, no
niri, no notify-send. It never runs or configures zellij at all: it answers "where is
session X", and `zsm` (later `wt <id>`) is what attaches with that in the environment. The
things reading `ZELLIJ_SESSION_NAME` are `zsm` and the zsh init, both through `wt path
--exact`.
